...
export class JenkinsService {
    ...
    public async getReplayScript(job: any, build: any) {
        return await vscode.window.withProgress({
            ...
            try {
                ...
                let r = await httpClient.get(url, { headers: this._headers });
    ...
    public async deleteBuild(job: any, buildNumber: any) {
        try {
            ...
            await httpClient.post(url, { headers: this._headers });
        } catch (err) {
    ...
    public async queueCancel(itemId: any) {
        try {
            ...
            let r = httpClient.post({ url: url, headers: this._headers });
            let output = await r;
            return output;
        } catch (err) {
            ext.logger.error(err);
